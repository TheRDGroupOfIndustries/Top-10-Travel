...
const ProgressBar = ({ activeStep, totalSteps }: Bar) => {
  ...
  return (
    <div className="relative w-[75%] sm:w-[70%] md:w-[70%] lg:w-[75%] max-w-[500px] mx-auto md:mt-[16%]  lg:mt-[11%] px-4  border-red-300 sm:mt-[5%] mt-[11%]">
      ...
      <div className="absolute flex justify-between w-full top-1/2 transform -translate-y-1/2">
        {icons.map((Icon, index) => (
          ...
          >
            <div
              className={` -mt-[50%] w-fit h-fit p-3 rounded-full flex justify-center items-center ${index <= activeStep ? 'bg-[#FCB62E]' : 'bg-[#ffd79c]'} -translate-x-4`}
            >
              <Icon className={`mx-auto text-2xl ${index <= activeStep ? 'text-white' : 'text-white'}`} />
            </div>
            <div className={`text-sm ${index <= activeStep ? 'text-[#FCB62E]' : 'text-gray-500'} -translate-x-4`}>
              {index === activeStep && (
