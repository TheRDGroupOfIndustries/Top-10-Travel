...
export default function DashboardListingDmc({
  listings,
  setSelectedDmc,
}: {
  listings: Prisma.DMCGetPayload<{
    include: {
  ...
  setSelectedDmc: (listing: any) => void;
}) {
              ...
              <DropdownMenuItem asChild>
                <Dialog>
                  <DialogTrigger asChild>
                    <Button
                      variant="ghost"
                      onClick={() => setSelectedDmc(listing)}
  ...
  return (
    <Card className="mt-5 z-[999] p-4 bg-[#F3F3F3]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-5">
          <h2 className="lg:text-3xl md:text-2xl text-xl font-semibold">
            <span className="text-mainColor">Dmc</span> created by Admin
          </h2>
      ...
      <div className="flex items-center">
        ...
        <Link href={`/auth/dmc`} className="ml-auto inline-block">
          <Button
            variant="outline"
            className="bg-[#F3F3F3] hover:bg-[#dbdbdb] border-mainColor"
          >
            Add Dmc <Plus className="ml-2 h-4 w-4" />
          </Button>
        </Link>
